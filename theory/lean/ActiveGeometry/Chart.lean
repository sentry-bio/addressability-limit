/-
  The space-form chart (optional)
  ===============================

  `h_pack` is a rate in nats per unit radius: a coordinate-free quantity. This
  file converts it into geometers' units, and that is all it does.

  If - and only if - one is willing to assume the host is isotropic and a space
  form, that rate has a name in Riemannian geometry, `(n-1)√κ`. The conversion
  imposes no new constraint and discovers nothing about the host; it is a
  dictionary entry. Read that way, the gauge lemmas below stop looking like
  physics and become what they are: the unit-consistency conditions of the
  dictionary.

  Nothing in `Capacity`, `Packing`, or `Constrained` depends on this file. The
  identification `hcap_eq_spaceForm` is a PREDICATE, assumed where used, never
  derived. Entering this namespace is an explicit act.
-/

import ActiveGeometry.Capacity

namespace ActiveGeometry.Chart
